...
- **站点统计展示**：顶部会显示当前注册用户数、成功生成图片数、当前正在进行的生图任务数，以及服务器内存、CPU、硬盘占用；前端每 3 秒刷新一次。成功生成数只统计 AI 文生图、图生图、局部重绘、去背景、删除物体、多角度等生成/编辑结果，上传图片、社媒/自媒体导入图片和裁剪图不会计入，方便开源部署者观察真实 AI 使用情况和服务器压力。

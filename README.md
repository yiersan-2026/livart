...
右侧对话框现在统一走 Agent 入口：用户可以直接问系统怎么用，也可以提出生成或编辑需求。Agent 会先判断意图，必要时检索 livart 系统知识库，再决定是回答问题、拒绝无关问题，还是创建一个或多个图片任务。所有普通回答都会自动排版成更易读的对话文本，长文本会拆成段落，编号和列表会转成结构化条目，错误和失败信息会以醒目的提醒样式显示，避免把大段原始文本直接堆给用户。
...
- **站点统计展示**：顶部会显示当前注册用户数和成功生成图片数，每次进入或刷新页面时通过后端统计接口重新查询；成功生成数只统计 AI 文生图、图生图、局部重绘、去背景、删除物体、多角度等生成/编辑结果，上传图片、社媒/自媒体导入图片和裁剪图不会计入，方便开源部署者观察真实 AI 使用情况。
- **Agent 对话入口**：右侧输入框统一提交到 `/api/agent/runs`，由 Agent 判断用户是在咨询系统功能、请求生图、做图生图编辑、删除物体、去背景、图层拆分还是局部重绘。
...
- **多角度 / 改视角**：选中图片后点击 `多角度`，弹出 Lovart 风格 3D 球面摄像机面板；主体图片保持固定，外层轨道球体套在图片中心并随拖拽旋转，3D 摄像机模型固定在球面某个位置并跟随球体运动，角度限制为左右各 `90°`、上方最高 `60°`、下方最低 `-30°`，最终按摄像机停留的方位角、俯仰角和缩放参数生成同一主体的新视角派生图。
...
- **WebSocket 生图状态推送**：生图任务统一由 `/api/agent/runs` 提交，运行、排队、完成和失败状态通过 `/ws/image-jobs` 实时推送；失败会同步显示在右侧对话和画布占位图上，不再用轮询兜底；右侧输入框会显示正在执行的任务耗时，默认 `16` 个图片 worker 并发处理多用户、多图片任务，超过并发上限的任务会进入队列并在对话框显示排队位置。
...
- **社交媒体图片导入**：底部工具栏新增“社媒图片”入口，支持抖音、小红书、微博、哔哩哔哩、知乎链接，后端代理解析图片列表，用户选择后导入画布并保存为本站图片资源；解析接口 Key 可通过 GoGoTool 的开放接口自行生成，不需要把共享密钥写进前端或仓库。
- **并行图片处理**：多个图片的重绘状态互不阻塞，切换图片不会清空各自输入内容。
- **永久画布后端**：新增 Spring Boot + MyBatis Plus 后端，用 PostgreSQL 保存画布项目，用 MinIO 保存图片资源。
- **站点概览统计**：后端提供站点级用户数和 AI 成功生成图片数统计，前端刷新页面后自动拉取并展示在顶部操作区；上传素材、社媒/自媒体导入素材和裁剪派生图会被排除。
...
多角度改视角参考 Lovart 的球面摄像机交互方式：点击 `多角度` 后会打开参数面板，中间是包围主体的 3D 圆球。主体图片位于中心固定层，不会跟着拖拽旋转；外层经纬线球体使用 3D 透视旋转并套在图片中心，摄像机是带机身、镜头、侧面厚度和顶部结构的 3D 小模型，固定在球面某个位置并随球体运动。用户可以拖动球面让摄像机在受限范围内移动：左右各 `90°`，上方最高 `60°`，下方最低 `-30°`；也可以精确调整 `方位角 / 俯仰角 / 缩放`。点击 `立即使用` 后会走同一套 Agent 图片编辑链路，提示词会明确告诉模型“最终 3D 球面摄像机停留位置”，尽量保持原图主体身份、材质、颜色、背景风格和画幅比例一致，只生成新的拍摄视角派生图。当前它是基于图生图模型的参数化视角编辑，不是严格 3D 重建。
...
项目根目录提供了多阶段 `Dockerfile` 和自包含的 `docker-compose.yml`：Compose 会同时启动 livart、带 pgvector 的 PostgreSQL、RabbitMQ 和 MinIO。浏览器访问同一个 Spring Boot 服务即可，前端统一提交到 `/api/agent/runs`，由后端 Agent 判断回答、拒绝或创建图片任务；系统功能问答会先检索 PostgreSQL 向量知识库，提示词优化已经内置在后端生图链路中，生图任务状态通过同域 `/ws/image-jobs` 推送。
...
底部工具栏的“社媒图片”按钮会打开导入弹窗。当前支持抖音、小红书、微博、哔哩哔哩和知乎链接。用户粘贴帖子链接后，前端只请求本站 `/api/external/images`，由后端携带第三方接口 Key 调用外部解析服务，避免 Key 暴露到浏览器。解析返回的图片会在弹窗内按原始比例预览，用户可多选并导入到当前画布；导入后图片会保存进 MinIO，同时生成 WebP 预览档位。

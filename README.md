...
- **多角度 / 改视角**：选中图片后点击 `多角度`，弹出 Lovart 风格 3D 球面摄像机面板；摄像机贴在主体外侧圆球表面，可水平 720 度拖动，最终按摄像机停留的方位角、俯仰角和缩放参数生成同一主体的新视角派生图。
...
多角度改视角参考 Lovart 的球面摄像机交互方式：点击 `多角度` 后会打开参数面板，中间是包围主体的 3D 圆球，摄像机图标贴在球面上。用户可以拖动球面让摄像机在前半球、侧面、背面和高低机位之间移动，水平范围支持 `-360° ~ 360°`，也可以精确调整 `方位角 / 俯仰角 / 缩放`。点击 `立即使用` 后会走同一套 Agent 图片编辑链路，提示词会明确告诉模型“最终 3D 球面摄像机停留位置”，尽量保持原图主体身份、材质、颜色、背景风格和画幅比例一致，只生成新的拍摄视角派生图。当前它是基于图生图模型的参数化视角编辑，不是严格 3D 重建。

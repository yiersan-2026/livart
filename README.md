...
- **多角度 / 改视角**：选中图片后点击 `多角度`，弹出 Lovart 风格 3D 球面观察面板；主体图片保持固定，外层经纬线球体套在图片中心并随拖拽旋转，球面上用左右对称的 3D 眼睛表示观察点，眼睛沿球面半径贴合在表面，连线指向眼睛中心；角度限制为左右各 `90°`、上方最高 `60°`、下方最低 `-30°`，重置回到 `0° / 0°`，点击 `应用` 后按观察点方位角、俯仰角和缩放参数生成同一主体的新视角派生图。
...
多角度改视角参考 Lovart 的球面观察交互方式：点击 `多角度` 后会打开参数面板，中间是包围主体的 3D 圆球。主体图片位于中心固定层，不会跟着拖拽旋转；外层经纬线球体使用 3D 透视旋转并套在图片中心，球面上的黑色点阵眼睛表示当前观察点。眼睛和连接线共用同一套 3D 球面坐标，观察点沿球体半径贴合在球面表面，连线始终指向眼睛中心。用户可以拖动球面让观察点在受限范围内移动：左右各 `90°`，上方最高 `60°`，下方最低 `-30°`；也可以精确调整 `方位角 / 俯仰角 / 缩放`，点击重置会回到 `0° / 0°`。点击 `应用` 后会走同一套 Agent 图片编辑链路，提示词会明确告诉模型“最终 3D 球面观察点停留位置”，尽量保持原图主体身份、材质、颜色、背景风格和画幅比例一致，只生成新的拍摄视角派生图。当前它是基于图生图模型的参数化视角编辑，不是严格 3D 重建。

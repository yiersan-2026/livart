...
右侧对话框现在统一走 Agent 入口：用户可以直接问系统怎么用，也可以提出生成或编辑需求。Agent 会先判断意图，必要时检索 livart 系统知识库，再决定是回答问题、拒绝无关问题，还是创建一个或多个图片任务。所有普通回答都会自动排版成更易读的 livart 卡片，长文本会拆成段落，编号和列表会转成结构化条目，错误和失败信息会以醒目的提醒样式显示，避免把大段原始文本直接堆给用户。
...
- **Agent 对话入口**：右侧输入框统一提交到 `/api/agent/runs`，由 Agent 判断用户是在咨询系统功能、请求生图、做图生图编辑、删除物体、去背景还是局部重绘。
- **格式化回答卡片**：系统回答不再裸露展示长文本，会自动拆分段落、识别中文/数字编号和项目列表，并用 livart 卡片、标签和错误提醒样式呈现。
...
- **WebSocket 生图状态推送**：生图任务统一由 `/api/agent/runs` 提交，运行、完成和失败状态通过 `/ws/image-jobs` 实时推送；失败会同步显示在右侧对话和画布占位图上，不再用轮询兜底；右侧输入框会显示正在执行的任务耗时，默认 `16` 个图片 worker 并发处理多用户、多图片任务。
...
图片生成任务默认使用 `LIVART_IMAGE_JOB_WORKER_COUNT=16`，适合 16 核服务器并行处理多用户任务；如果上游接口限流或 502/504 增多，可以在 `.env` 中把它降到 `8` 或 `12` 后重启服务。

...
- **站点统计展示**：顶部会显示当前注册用户数、成功生成图片数和当前正在进行的生图任务数；成功生成数只统计 AI 文生图、图生图、局部重绘、去背景、删除物体、多角度等生成/编辑结果，上传图片、社媒/自媒体导入图片和裁剪图不会计入，方便开源部署者观察真实 AI 使用情况。
...
- **站点概览统计**：后端提供站点级用户数、AI 成功生成图片数和当前排队/运行中的生图任务数统计，前端刷新页面和当前页面任务状态变化后会自动拉取并展示在顶部操作区；上传素材、社媒/自媒体导入素材和裁剪派生图会被排除。

...
- **多角度 / 改视角**：选中图片后点击 `多角度`，弹出 Lovart 风格 3D 球面观察面板；画面转向模式会把原图当作完整三维场景处理，观察视角模式会围绕整张图片内容移动观察点，要求前景、背景、道具、地面/室内结构、车辆、建筑、光源、阴影和反射按同一镜头角度产生一致透视变化，而不是只旋转或重绘主角。

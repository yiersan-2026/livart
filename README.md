...
- **多角度 / 改视角**：选中图片后点击 `多角度`，弹出 Lovart 风格 3D 球面观察面板；系统会把原图当作固定不动的完整三维场景，只移动相机/观察点。画面中所有可见元素（人物、动物、车辆、家具、道具、背景、地面/室内结构、建筑、光源、阴影和反射）都要按同一个新相机位姿产生一致透视变化；人物和物体保持原姿态、原头部朝向和原视线，不会要求主角追随新镜头。

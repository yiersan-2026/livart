...
- **多角度 / 改视角**：选中图片后点击 `多角度`，弹出 Lovart 风格参数面板，可在 `主体 / 摄像头` 两种模式之间切换，通过拖动 3D 预览或调整 `旋转 / 倾斜 / 缩放` 参数生成同一主体的新视角派生图。
...
多角度改视角参考 Lovart 的交互方式：点击 `多角度` 后会打开参数面板，顶部可切换 `主体` 或 `摄像头` 模式，中间 3D 预览可直接拖动来改变旋转和倾斜，下面也可以精确调整 `旋转 / 倾斜 / 缩放`。点击 `立即使用` 后会走同一套 Agent 图片编辑链路，尽量保持原图主体身份、材质、颜色、背景风格和画幅比例一致，只生成新的拍摄视角派生图。当前它是基于图生图模型的参数化视角编辑，不是严格 3D 重建。

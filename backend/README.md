...
本后端是 livart 在原开源前端画布项目基础上新增的永久画布服务层，负责账号登录、JWT 鉴权、用户中转站配置、项目管理、画布状态保存、图片资源上传、异步保存队列、WebSocket 生图状态推送、系统知识库检索和 AI Agent 执行。前端不再直接调用文生图/图生图提交接口，而是统一提交到 Agent，由 Agent 判断回答、拒绝或执行图片任务；普通系统问答会由前端排版成 livart 卡片，长段落、编号列表和错误提醒会自动结构化展示。系统功能问答会先检索 PostgreSQL 知识库，pgvector 可用时走向量检索，不可用时退回关键词检索；提示词优化不再暴露单独接口，而是在后端调用上游文生图/图生图接口前自动执行；`gpt-image-2` 文生图会按画幅默认注入 2K `size`；删除物体会使用专门的 `image-remover` 模式，避免普通重绘提示词影响局部删除；去背景会使用 `background-removal` 模式，先识别图片主要主体，再只保留主体并把主体以外区域替换为纯白色背景。异步生图任务会在 job 响应和 WebSocket 推送中返回 `originalPrompt` 与 `optimizedPrompt`，前端会随图片节点一起保存。
